...
- 认证参数：`api-key`
- 适用账号：企业主账号、企业子账号
- `api-key` 获取位置：个人中心的账号信息区域
- `api-key` 有效期：不会自动过期，可在个人中心手动更新
- 扣费口径：企业主账号或子账号调用接口后，扣除企业积分
...
企业版资产结果可包含 `whois`, `body`, `vul_list`。

## 批量查询接口

企业版批量查询支持两类结果形式：文件导出和数组拉取。接口支持通过搜索语法或 CSV 文件创建异步任务。
...
| `ip` | 仅 IP、IP 段精确检索 | 最多 10000 个目标 |
| `domain` | 仅域名检索，等效于 `domain.suffix` 语法 | 最多 10000 个目标 |
| `company` | 仅企业名称检索，等效于 `icp.name` 精确检索 | 最多 10000 个目标 |
...
```http
GET /openApi/search/batch/pull
```

用途：对批量查询任务结果进行分页拉取，返回数组形式的数据。
...
| --- | --- | --- |
| `api-key` | 是 | 企业主账号或子账号 API Key |
| `task_id` | 是 | 批量查询接口返回的任务 ID |
| `page` | 是 | 页码 |
| `page_size` | 是 | 每页资产条数，可选 `100`, `200`, `500`, `1000` |

#### 示例

```bash
curl -X GET -k "https://hunter.qianxin.com/openApi/search/batch/pull?api-key={api-key}&task_id={task_id}&page=1&page_size=500"
```
...
| `data.enterprise_info.account` | 企业账号 |
| `data.enterprise_info.start_time` | 订单开始时间 |
| `data.enterprise_info.end_time` | 订单结束时间 |

### 企业子账号响应字段

| 字段 | 说明 |
| --- | --- |
| `data.type` | 账号类型，企业子账号为 `企业子账号` |
| `data.rest_equity_point` | 剩余权益积分 |
| `data.rest_export_quota` | 当日剩余导出额度 |
| `data.day_export_quota` | 当日导出额度上限 |
| `data.once_export_quota` | 单次导出额度上限 |
| `data.personal_info.username` | 子账号用户昵称 |
| `data.personal_info.phone` | 子账号手机号 |
| `data.enterprise_info.account` | 企业主账号 |
| `data.enterprise_info.start_time` | 订单开始时间 |
| `data.enterprise_info.end_time` | 订单结束时间 |
